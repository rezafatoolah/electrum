...
class Peer(Logger):
    ...
    def on_channel_reestablish(self, chan, msg):
        ...
        if self.is_peerbackup_client():
            peerbackup = msg['channel_reestablish_tlvs']['your_last_state']
            peerbackup_bytes = peerbackup['state']
            local_signature = peerbackup['signature_local']
            remote_signature = peerbackup['signature_remote']
            local_sighash, remote_sighash = chan.verify_peerbackup_signatures(
                peerbackup_bytes, local_signature, remote_signature)
            if chan.compare_peerbackup_with_last_hash(peerbackup_bytes, local_sighash, remote_sighash):
                self.logger.info('received peerbackup matches our last sent')
            else:
                self.logger.info('received peerbackup does not match our last sent')
            #    chan.restore_state_from_peer_backup(peerbackup_bytes)
    ...
    async def reestablish_channel(self, chan: Channel):
        ...
        if self.is_peerbackup_server():
            channel_reestablish_tlvs['your_last_state'] = {
                'state': chan.get_their_reconstructed_peerbackup_bytes(),
                'signature_local': chan.get_their_peerbackup_signature(LOCAL),
                'signature_remote': chan.get_their_peerbackup_signature(REMOTE),
            }
